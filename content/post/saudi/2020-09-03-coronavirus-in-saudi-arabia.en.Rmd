...
saudi_death <- coronavirus %>%
  filter(country == "Saudi Arabia",
         type == "death") %>%
  mutate(previous_day_death = lag(cases),
         new_death = cases-previous_day_death,
         total_death = sum(cases)) %>%
  filter(date != "2020-01-22") %>%
  select(-country,
         -lat,
         -long,
         -type,
         -province) %>%
  summarise(average_death_cases = mean(new_death),
            max_death_cases = max(new_death),
            total = sum(cases)) 
```
...
The maximum number of new cases on any given day was `r  saudi_confirmed$max_new_cases` cases, which is approximately `r percent(saudi_confirmed$max_new_cases/2102, accuracy = 0.01)` of the total number of MERS-CoV infections in Saudi Arabia over an 8-year period, emphasising the magnitude of COVID-19. The average number of new infections per day is approximately `r saudi_confirmed$average_new_cases` cases, whilst the average number of recovered cases per day is `r saudi_recovered$average_recovered_cases` cases. The maximum number of recovered cases on a given day was `r saudi_recovered$max_recovered_cases` cases. 
...
Ultimately, the summary statistics and the interactive table suggest a clear difference in the prevalance of MERS-CoV and COVID-19. Although MERS-CoV is much more fatal with an alarmingly high death rate, COVID-19 is much more infectious. 
